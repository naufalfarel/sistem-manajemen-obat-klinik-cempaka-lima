...
class LaporanController extends Controller
{
    ...
    public function stok(Request $request): JsonResponse|StreamedResponse
    {
        ...
        if ($format === 'pdf') {
            return $this->pdf(
                ...
                subtitle: 'Seluruh obat aktif per '.now()->toDateString(),
    ...
    public function kadaluarsa(Request $request): JsonResponse|StreamedResponse
    {
        ...
        if ($format === 'pdf') {
            ...
            return $this->pdf(
                title: 'Laporan Obat Kadaluarsa',
                subtitle: 'Seluruh obat aktif dengan tanggal expired per '.now()->toDateString(),
                headings: $export->headings(),
                rows: $export->collection()->map(fn ($o) => $export->map($o)),
     ...
     */
    private function pdf(string $title, string $subtitle, array $headings, Collection $rows, string $filename): StreamedResponse
    {
    ...
    public function logistik(Request $request): JsonResponse|StreamedResponse
    {
        $dari = $request->string('dari')->value();
        $sampai = $request->string('sampai')->value();
